...
package dev.apexstudio.ide.actions.filetree
...
import dev.apexstudio.ide.utils.FlashType
import dev.apexstudio.ide.utils.flashMessage
...
class RenameAction(context: Context, override val order: Int) :
  ...
  override suspend fun execAction(data: ActionData) {
    ...
    binding.name.editText!!.hint =
      context.getString(dev.apexstudio.ide.resources.R.string.new_name)
    binding.name.editText!!.setText(file.name)
    builder.setTitle(dev.apexstudio.ide.resources.R.string.rename_file)
    builder.setMessage(dev.apexstudio.ide.resources.R.string.msg_rename_file)
    builder.setView(binding.root)
    builder.setNegativeButton(android.R.string.cancel, null)
    builder.setPositiveButton(dev.apexstudio.ide.resources.R.string.rename_file) {
      dialogInterface,
      _ ->
      dialogInterface.dismiss()
      actionScope.launchAsyncWithProgress(
          configureFlashbar = { builder, cancelChecker ->
            builder.message(dev.apexstudio.ide.resources.R.string.please_wait)
          },
          action = { _, _ ->
            ...
            withContext(Dispatchers.Main) {
              flashMessage(
                  if (renamed) dev.apexstudio.ide.resources.R.string.renamed
                  else dev.apexstudio.ide.resources.R.string.rename_failed,
