...
package dev.apexstudio.ide.javac.services.partial

import dev.apexstudio.ide.javac.services.compiler.JavacFlowListener
import dev.apexstudio.ide.javac.services.util.ReparserUtils
import dev.apexstudio.ide.javac.services.visitors.FindAnonymousVisitor
import dev.apexstudio.ide.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = dev.apexstudio.ide.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
      dev.apexstudio.ide.javac.services.NBParserFactory.instance(context)
        as dev.apexstudio.ide.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = dev.apexstudio.ide.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
      endPositions
        is dev.apexstudio.ide.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
      dev.apexstudio.ide.javac.services.NBParserFactory.NBJavacParser(
        factory,
        lexer,
        true,
        false,
