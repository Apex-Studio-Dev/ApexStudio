...
import dev.apexstudio.ide.lsp.java.Rewrite.ImplementAbstractMethods
...
class ImplementAbstractMethodsAction : BaseJavaCodeAction() {
  ...
  override fun prepare(data: ActionData) {
    ...
    if (!data.hasRequiredData(dev.apexstudio.ide.lsp.models.DiagnosticItem::class.java)) {
      markInvisible()
      return
    }

    val diagnostic = data.get(dev.apexstudio.ide.lsp.models.DiagnosticItem::class.java)!!
  ...
  override suspend fun execAction(data: ActionData): Any {
    val diagnostic =
      JavaDiagnosticUtils.asJCDiagnostic(
        data.get(
          dev.apexstudio.ide.lsp.models.DiagnosticItem::class.java)!!.extra as Diagnostic<out JavaFileObject>
      )
