...
import dev.apexstudio.ide.fragments.BaseFragment
import dev.apexstudio.ide.inflater.IView
import dev.apexstudio.ide.inflater.internal.AttributeImpl
import dev.apexstudio.ide.inflater.internal.LayoutFile
import dev.apexstudio.ide.inflater.internal.ViewGroupImpl
...
import dev.apexstudio.ide.uidesigner.R
import dev.apexstudio.ide.uidesigner.UIDesignerActivity
import dev.apexstudio.ide.uidesigner.databinding.FragmentDesignerWorkspaceBinding
import dev.apexstudio.ide.uidesigner.drag.WidgetDragListener
import dev.apexstudio.ide.uidesigner.drag.WidgetTouchListener
import dev.apexstudio.ide.uidesigner.drawable.UiViewLayeredForeground
import dev.apexstudio.ide.uidesigner.models.CommonUiView
import dev.apexstudio.ide.uidesigner.models.PlaceholderView
import dev.apexstudio.ide.uidesigner.models.RootWorkspaceView
import dev.apexstudio.ide.uidesigner.models.UiViewGroup
import dev.apexstudio.ide.uidesigner.undo.UndoManager
import dev.apexstudio.ide.uidesigner.utils.UiLayoutInflater
import dev.apexstudio.ide.uidesigner.utils.applyPreview
import dev.apexstudio.ide.uidesigner.utils.bgDesignerView
import dev.apexstudio.ide.uidesigner.utils.layeredForeground
import dev.apexstudio.ide.uidesigner.viewmodel.M3ComponentState
import dev.apexstudio.ide.uidesigner.viewmodel.WorkspaceViewModel
import java.io.File
...
class DesignerWorkspaceFragment : BaseFragment() {
  ...
  private fun createM3ComponentFromState(m3Component: M3ComponentState): IView? {
    return try {
      ...
      val layoutFile = dev.apexstudio.ide.inflater.internal.LayoutFile(File(""), "")
      val iView =
          when (view) {
            is MaterialButton -> {
              // Use ViewImpl instead of checking the actual view type
              dev.apexstudio.ide.inflater.internal.ViewImpl(layoutFile, m3Component.type, view)
            }
            is MaterialCardView -> {
              dev.apexstudio.ide.inflater.internal.ViewGroupImpl(layoutFile, m3Component.type, view)
            }
            is MaterialTextView -> {
              dev.apexstudio.ide.inflater.internal.ViewImpl(layoutFile, m3Component.type, view)
            }
        ...
        try {
          val namespace =
              if (attrName.startsWith("app:")) {
                dev.apexstudio.ide.inflater.INamespace.APP
              } else {
                dev.apexstudio.ide.inflater.INamespace.ANDROID
              }
          val attr =
              dev.apexstudio.ide.inflater.internal.AttributeImpl(
                  namespace = namespace,
