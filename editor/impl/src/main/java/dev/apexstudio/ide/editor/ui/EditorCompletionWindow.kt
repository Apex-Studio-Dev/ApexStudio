...
import dev.apexstudio.ide.lsp.util.DocumentationReferenceProvider
import dev.apexstudio.ide.progress.ProgressManager
import dev.apexstudio.ide.utils.KeyboardUtils
  ...
  override fun setLayout(layout: CompletionLayout) {
    ...
    (layout.completionList as? ListView)?.let {
      ...
      it.setOnItemLongClickListener { _, view, position, _ ->
        val data =
          (items[position] as? dev.apexstudio.ide.lsp.models.CompletionItem)?.data
